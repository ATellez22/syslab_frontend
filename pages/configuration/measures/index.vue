                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(elemento, indice) in list">
                      ...
                      <td class="py-0 px-1">{{ elemento.code }}</td>
                      <td class="py-0 px-1">
                        <div class="btn-group">
  ...
  methods: {
    ...
    async EliminarItem(id) {
      try {
        ...
        this.load = true;
        const res = await this.$api.$delete("measures/" + id);
  ...
  mounted() {
    ...
    this.$nextTick(async () => {
      ...
      */
      ...
      try {
        await Promise.all([this.GET_DATA("measures")]).then((position) => {
          //La promesa resuelta 'position' se trasfiere a 'list' desde su primer valor '0'.
          this.list = position[0];
        });
      } catch (error) {
