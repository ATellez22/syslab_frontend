              ...
              <div class="card-body">
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(elemento) in list">
                      <td class="py-0 px-1">{{ elemento.id }}</td>
                      <td class="py-0 px-1">{{ elemento.description }}</td>
                      <td class="py-0 px-1">{{ elemento.barcode }}</td>
                      <td class="py-0 px-1">
                        {{ elemento.brand.description }}
                      </td>
                      <td class="py-0 px-1">
                        {{ elemento.category.description }}
                      </td>
                      <td class="py-0 px-1"><span
                      class="badge"
                      :class="[
                        elemento.stock < elemento.minimum_stock
                          ? 'badge-danger'
                          : 'badge-success',
                      ]"
                      >
                      {{ elemento.stock }} {{ elemento.measure.code }}</span></td>
                      <td class="py-0 px-1">{{ Number(elemento.investment).toFixed(2) }} </td>
                      <td class="py-0 px-1">{{ Number(elemento.valued).toFixed(2) }} </td>
                      <td class="py-0 px-1">{{ Number(elemento.profit).toFixed(2) }} </td>
                      <td class="py-0 px-1">
                        <div class="btn-group">
  ...
  methods: {
    ...
    async EliminarItem(id) {
      try {
        ...
        this.load = true;
        const res = await this.$api.$delete("articles/" + id);
