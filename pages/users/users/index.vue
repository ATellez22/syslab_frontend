                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(elemento, indice) in list">
                      ...
                      <td class="py-0 px-1">{{ elemento.name }}</td>
                      <td class="py-0 px-1">{{ elemento.email }}</td>
                      <td class="py-0 px-1">{{ elemento.username }}</td>
                      <td class="py-0 px-1">
                        <div class="btn-group">
  ...
  mounted() {
    ...
    this.$nextTick(async () => {
      ...
      */
      ...
      try {
        await Promise.all([this.GET_DATA("users")]).then((position) => {
          //La promesa resuelta 'position' se trasfiere a 'list' desde su primer valor '0'.
          this.list = position[0];
        });
      } catch (error) {
